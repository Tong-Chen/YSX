...
flower_plot(set_data, item_variable = "Gene", set_variable = "Sample")
...
```{r}
sp_upsetview(vennDiagram_data, vennFormat = 2, saveplot="1.pdf", width=10, height=4)
```
...
sp_barplot(data = bar_data, xvariable = "ID", yvariable = "Exper", color_variable = "Gene",height=12.36,width=20,filename = "s.pdf")
```
...
sp_barplot(data = box_demo4_data, melted = T, 
...
sp_raincloud(data = data,xvariable = "Condition", yvariable = "Value", legend_variable = "Condition",position_nudge_flat_violin_x = 0.3)
...
# 整理数据
colnames(my_data)[1] <- c("Participant") 
library(reshape2)
my_datal <- melt(my_data, id.vars = c("Participant"), 
                 measure.vars = c("AngerUH", "DisgustUH", "FearUH", "HappyUH"), 
                 variable.name = "EmotionCondition", value.name = "Sensitivity")
sp_raincloud(data= my_datal,xvariable= "EmotionCondition",yvariable= "Sensitivity",legend_variable="EmotionCondition")
...
```{r}
plants[c("erigenia", "solanum", "cynodon")]
plot(euler(plants[c("erigenia", "solanum", "cynodon")]))
```
...
sp_EulerDiagrams(data = data , saveplot = "euler.pdf" )
```
