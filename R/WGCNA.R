...
WGCNA_readindata <- function(exprMat, traitData=NULL, categoricalTrait=NULL,
  ...
  datExpr <- read.table(exprMat, sep=sep, row.names=row.names, header=header,
...
#' dataFilter(df)
#'
dataFilter <- function(datExpr, minimal_mad=NULL, top_mad_n=0.75, rmVarZero=T) {

  m.mad <- apply(datExpr,1,mad)

  if(!is.null(minimal_mad)){
...
WGCNA_dataFilter <- function (datExpr, ...){
  ...
  cat("After filter,", nGenes, 'genes x', nSamples, "samples remained.")
...
#'
WGCNA_sampleClusterDetectOutlier <- function(datExpr, thresholdZ.k = -2.5,
                                             saveplot=NULL,
                                             removeOutlier=F, traitColors=NULL, ...){
  ## 样本层级聚类，查看有无离群值
...
#' and 40 for other type.
#' @param RsquaredCut R2 for defining scale-free network.
#'
#' @return A number
#' @export
#'
...
#' power <- WGCNA_softpower(datExpr)
#'
WGCNA_softpower <- function(datExpr, networkType="signed", saveplot=NULL, maxPower=NULL,
                            RsquaredCut = 0.85, ...) {
  ## 软阈值筛选
...
WGCNA_coexprNetwork <- function(datExpr, power, maxBlockSize = NULL,
  ...
  if(!is.null(saveplot)) {
    base_plot_save(saveplot, width=width, height=height, bg="white", ...)
  }
...
#'
#'
WGCNA_saveModuleAndMe <- function(net, datExpr, prefix="ehbio", saveplot=NULL, ...){
  ## 共表达网络结果输出
  ...
  write.table(gene_module,file=paste0(prefix,".gene_module.xls"),
  ...
  write.table(MEs_colt,file=paste0(prefix,".module_eipgengene.xls"),
              sep="\t",quote=F)
...
#'
WGCNA_MEs_traitCorrelationHeatmap <- function(MEs_col, traitData, saveplot=NULL, ...) {
  if(!is.null(saveplot)) {
    base_plot_save(saveplot, bg="white", ...)
  }
...
WGCNA_cytoscape <- function(net, power, datExpr, TOM_plot=NULL, prefix="ehbio", fulledge=F){
  ...
  write.table(edgeData, paste0(prefix, ".cytoscape_edges_within_modules.txt"),
              quote=F, sep="\t", row.names=F)
...
#'
#' @param edgeData generated by \code{WGCNA_cytoscape}
#' @param nodeData generated by \code{WGCNA_cytoscape}
#' @param module A module color name
...
#' None
#'
WGCNA_cytoscape_each_module <- function(edgeData, nodeData, module, prefix){
  if (module != "grey") {
    nodeData <- nodeData[nodeData$Module == module,]
    nodes = nodeData$nodeName
    edgeData <- edgeData[(edgeData$Source %in% nodes) & (edgeData$Target %in% nodeData$nodes), ]
    write.table(nodeData, paste(prefix, module,"cytoscape_nodes.txt",sep="."),
                quote=F, sep="\t", row.names=F)
    write.table(edgeData, paste(prefix, module,"cytoscape_edges.txt",sep="."),
                quote=F, sep="\t", row.names=F)
  }
}
...
#' hubgene <- WGCNA_hubgene(cyt)
#'
WGCNA_hubgene <- function(cyt, top_hub_n=10, prefix='ehbio'){
  #library(dplyr)
  ...
  nodeTotalWeight <- edgeData %>% dplyr::group_by(Node1, Module1) %>% dplyr::summarise(weight=sum(Weight))
  ...
  write.table(nodeTotalWeightTop20, paste(prefix,"hubgenes.txt",sep="."),
              quote=F, sep="\t", row.names=F)
...
#'
#'
WGCNA_moduleTraitPlot <- function(MEs_col, traitData, corType="bicor", saveplot=NULL,
                                  prefix='ehbio', ...){
  ### 模块与表型数据关联
  if(is.null(traitData)) {
    print("Trait data needed.!!!!")
    invisible(1)
  }
  ...
  write.table(modTraitCorMelt,file=paste0(prefix,".module_trait_correlation.xls"),
              sep="\t",quote=F)
  ...
  write.table(modTraitPMelt,file=paste0(prefix,".module_trait_correlationPvalue.xls"),
              sep="\t",quote=F)
  ...
  write.table(modTraitCorP,file=paste0(prefix,".module_trait_correlationPvalueMelt.xls"),
              sep="\t",quote=F,row.names=F)
}
...
WGCNA_ModuleGeneTraitHeatmap <- function(datExpr, traitData, net, corType="bicor",
  ...
  if (corType=="pearson") {
    ...
    geneTraitP = as.data.frame(WGCNA::corPvalueStudent(
  ...
  write.table(geneTraitPMelt,file=paste0(prefix,".gene_trait_correlationPvalue.xls"),
              sep="\t",quote=F)
...
#'
WGCNA_GeneModuleTraitCoorelation <- function(datExpr, MEs_col, geneTraitCor,
                                             traitData, net, corType="bicor",
                                             prefix="ehbio", ...) {
  ### 计算模块与基因的相关性矩阵
  robustY = ifelse(corType=="pearson",T,F)
  if (corType=="pearson") {
    geneModuleMembership = as.data.frame(cor(datExpr, MEs_col, use = "p"))
    MMPvalue = as.data.frame(WGCNA::corPvalueStudent(
      as.matrix(geneModuleMembership), nSamples))
  } else {
    # 关联样品性状的二元变量时，设置
    geneModuleMembershipA = WGCNA::bicorAndPvalue(datExpr, MEs_col, robustY=robustY)
...
#' @inheritParams WGCNA_coexprNetwork
#' @inheritParams WGCNA_cytoscape
#' @param os_system Default the program will detect system type to choose which multiple thread function will be used. <enableWGCNAThreads> is recommended, but only work in some linux os. <allowWGCNAThreads> is not recommended if enableWGCNAThreads works. However for max and windows os, this is the only one can be used. Even for some linux system, using <enableWGCNAThreads> will make programs stuck in <pickSoftThreshold> step. So if stucked, supply any string other than <linux> to enable the uages of <allowWGCNAThreads>.
...
#'
WGCNA_onestep <- function(exprMat, traitData=NULL, categoricalTrait=NULL,
                          prefix="ehbio", corType="bicor", networkType="signed",
                          maxPower=NULL, maxBlockSize=NULL,
                          top_mad_n=0.75, rmVarZero=T, minimal_mad=NULL,
                          thresholdZ.k=-2.5, TOM_plot=NULL, top_hub_n=10,
                          removeOutlier=F, RsquaredCut=0.85,
                          minModuleSize = 25, mergeCutHeight = 0.2,
                          numericLabels = TRUE, pamRespectsDendro = FALSE,
                          saveTOMs=TRUE, maxPOutliers=NULL, loadTOM=TRUE,
                          TOMDenom = "min",  deepSplit = 1,
                          stabilityCriterion = "Individual fraction",
                          verbose = 3, os_system=NULL,
                          randomSeed=11521, dynamicCutPlot=TRUE, power_min=NULL, ...){

  options(stringsAsFactors = FALSE)

  if (is.null(os_system)) {
  ...
  WGCNA_dataCheck(datExpr, saveplot=paste0(prefix,".WGCNA_dataCheck.pdf"), width=20)

  datExpr <- WGCNA_dataFilter(datExpr, minimal_mad=minimal_mad, top_mad_n=top_mad_n,
                              rmVarZero=rmVarZero)
  ...
  if (!is.null(power_min) & (power < power_min)){
    power = power_min
  }
  ...
  WGCNA_moduleTraitPlot(MEs_col, traitData=wgcnaL$traitData,
  ...
  WGCNA_GeneModuleTraitCoorelation(datExpr, MEs_col, geneTraitCor,
                                   traitData=wgcnaL$traitData, net, corType=corType,
                                   prefix=prefix)
